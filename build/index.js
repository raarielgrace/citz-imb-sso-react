import require$$0, { createContext, useContext, useMemo, useEffect, useState, useReducer } from 'react';
...
const normalizeUser = (userInfo) => {
    const { name = '', preferred_username, email, display_name, client_roles = [], scope = '', identity_provider, } = userInfo;
    let guid;
    let username;
    let first_name;
    let last_name;
...
var name = "@bcgov/citz-imb-sso-react";
var version = "1.1.0-beta";
var description = "BCGov SSO integration for React";
var author = "CITZ IMB Common Code <citz.codemvp@gov.bc.ca>";
var license = "Apache-2.0";
var main = "build/index.js";
var types = "build/bundle.d.ts";
var type = "module";
var scripts = {
...
var homepage = "https://github.com/bcgov/citz-imb-sso-react#readme";
var packageJson = {
	name: name,
	version: version,
...
const RefreshExpiryDialog = (props) => {
    ...
    if (!isVisible)
        return null;
    return (jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [jsxRuntimeExports.jsx("div", { className: "ssor_dialog-overlay" }), jsxRuntimeExports.jsx("dialog", { className: "ssor_dialog", open: isVisible, children: jsxRuntimeExports.jsxs("div", { className: "ssor_dialog-content", children: [jsxRuntimeExports.jsx("p", { className: "ssor_dialog-message", children: message }), jsxRuntimeExports.jsx("button", { className: "ssor_button", onClick: () => login(loginProps), children: buttonText })] }) })] }));
};

const SSOProvider = (props) => {
    const { children, backendURL, idpHint, onRefreshExpiry, overrideShowRefreshExpiryDialog, postLoginRedirectURL, refreshExpiresInOffset, } = props;
    const [isExpiryDialogVisible, setIsExpiryDialogVisible] = useState(false);
    const [state, dispatch] = useReducer(reducer, initialState);
    useEffect(() => {
        checkForUpdates();
    }, []);
    return (jsxRuntimeExports.jsxs(AuthContext.Provider, { value: { state, dispatch }, children: [jsxRuntimeExports.jsx(SSOWrapper, { backendURL: backendURL, onRefreshExpiry: onRefreshExpiry ? () => onRefreshExpiry() : () => setIsExpiryDialogVisible(true), refreshExpiresInOffset: refreshExpiresInOffset, children: children }), jsxRuntimeExports.jsx(RefreshExpiryDialog, { loginProps: { backendURL, idpHint, postLoginRedirectURL }, isVisible: overrideShowRefreshExpiryDialog ? true : isExpiryDialogVisible })] }));
};

export { AuthContext, SSOProvider, SSOWrapper, decodeJWT, useSSO };
